...
export default function DeleteLessonDialog({ lesson, isOpen, onClose, onConfirm }: DeleteLessonDialogProps) {
    ...
    try {
      ...
      if (!response.ok || !result.success) {
        if (response.status === 401 || response.status === 403) {
          ...
          setErrorDialogMessage(result.error || 'Không thể xóa lesson');
          setShowErrorDialog(true);
        }
        return;
      }
          ...
          <div className="flex items-start gap-4 mb-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-600 mb-2">
                Bạn có chắc chắn muốn xóa lesson <strong>"{lesson.title}"</strong>?
              </p>
